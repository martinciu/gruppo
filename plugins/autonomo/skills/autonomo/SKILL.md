...
>     - `event=assumption question="<one line>" answer="<one line>"` the *moment* you make a best-effort scope-ambiguity call (rule 1), in addition to surfacing the same `Q:` / `A:` pair in the `## Assumptions` section of your final return. **The two surfaces are 1:1: every Q in your final spec's `## Assumptions` section must have a matching log line, including assumptions you only realize you made during a later stage like `write`.** Emit the log line under whatever stage you are currently in when you notice the call; do not retroactively reorder under `clarify`.
>
